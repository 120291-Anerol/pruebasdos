							...
							if (allow_order == false)
							{
								if ( (output[0][1] <= deny_order) || (output[0][1] <= 0) )
								{
									...
									self.showPopup('ErrorPopup', {
										title: self.env._t('Super barato'),
										body: self.env._t("Orden detenida, " + "(" + product.display_name + ")" + " no tiene Stock."),
									});
								}
							}
					...
					if (pos_config.show_stock_location == 'specific')
					{
						...
						await this.rpc({
							...
							if(flag > 0){
								...
								self.showPopup('ErrorPopup', {
									title: self.env._t('Alerta'),
									body: self.env._t('Uno o mas productos no tienen stock.'),
								});
							}
						});
					} else {
